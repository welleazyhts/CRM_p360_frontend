...
const CaseDetails = () => {
  ...
  const handleSaveEdit = async () => {
    try {
      // Import the API function to update case
      const { updateCase } = await import('../services/api');

      // Prepare update data
      const updateData = {
        status: editFormData.status,
        subStatus: editFormData.subStatus,
        agent: editFormData.agent,
        nextFollowUpDate: editFormData.nextFollowUpDate,
        nextActionPlan: editFormData.nextActionPlan,
        currentWorkStep: editFormData.currentWorkStep,
        contactInfo: {
      ...
      const newHistoryEntry = {
        date: new Date().toISOString(),
        action: 'Case Updated',
        details: `Case details updated by Current User`,
        user: 'Current User'
      };
        ...
        >
          <DialogTitle>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Typography variant="h6">Edit Case - {caseData?.id}</Typography>
            </Box>
          </DialogTitle>
          <DialogContent>
            <Grid container spacing={3} sx={{ mt: 1 }}>
              ...
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth>
                  ...
                  <Select
                    value={editFormData.subStatus}
                    label="Sub-Status"
                    onChange={(e) => setEditFormData(prev => ({ ...prev, subStatus: e.target.value }))}
                  >
                    <MenuItem value="Document Pending">Document Pending</MenuItem>
                    <MenuItem value="Customer Contact Required">Customer Contact Required</MenuItem>
                    <MenuItem value="Payment Processing">Payment Processing</MenuItem>
                    <MenuItem value="Verification in Progress">Verification in Progress</MenuItem>
                    <MenuItem value="Approval Required">Approval Required</MenuItem>
                    <MenuItem value="Ready for Renewal">Ready for Renewal</MenuItem>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) => setEditFormData(prev => ({ ...prev, nextFollowUpDate: e.target.value }))}
              ...
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth>
                  ...
                  <Select
                    value={editFormData.currentWorkStep}
                    label="Current Work Step"
                    onChange={(e) => setEditFormData(prev => ({ ...prev, currentWorkStep: e.target.value }))}
                  >
                    <MenuItem value="Initial Contact">Initial Contact</MenuItem>
                    <MenuItem value="Document Collection">Document Collection</MenuItem>
                    <MenuItem value="Verification">Verification</MenuItem>
                    <MenuItem value="Premium Calculation">Premium Calculation</MenuItem>
                    <MenuItem value="Payment Processing">Payment Processing</MenuItem>
                    <MenuItem value="Policy Generation">Policy Generation</MenuItem>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  multiline
                  rows={3}
                  value={editFormData.nextActionPlan}
                  onChange={(e) => setEditFormData(prev => ({ ...prev, nextActionPlan: e.target.value }))}
                  placeholder="Enter the next action plan for this case..."
                />
              </Grid>
